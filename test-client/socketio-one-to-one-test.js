...
class OneToOneChatTester {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('connect', () => {
        ...
        socket.emit('authenticate', { token }, (response) => {
          if (response.success) {
            this.log(`User ${userNum} authenticated as ${response.userId}`);
            if (userNum === 1) {
      ...
      socket.on('error', (error) => {
        this.log(`User ${userNum} socket error: ${error.message}`, 'error');
      });
  ...
  setupEventListeners(socket, userNum) {
    ...
    socket.on('authenticated', (data) => {
      this.log(`User ${userNum} authentication confirmed: ${data.message}`);
    });
    ...
    socket.on('newDirectMessage', (data) => {
      this.log(`User ${userNum} received message: "${data.message.content}" from ${data.message.sender.name}`);
    ...
    socket.on('messagesRead', (data) => {
      this.log(`User ${userNum} - Messages read by ${data.readBy.name} in conversation ${data.conversationId}`);
    });
    ...
    socket.on('incomingCall', (data) => {
      this.log(`User ${userNum} - Incoming ${data.callType} call from ${data.caller.name}`);
    ...
    socket.on('callResponse', (data) => {
      this.log(`User ${userNum} - Call ${data.response}ed by ${data.responder.name}`);
    });
    ...
    socket.on('userStatusChanged', (data) => {
      this.log(`User ${userNum} - User ${data.userId} status changed to ${data.status}`);
    });
  }
  ...
  async runAllTests() {
    ...
    try {
      ...
      await this.runTest('User Connection and Authentication', async () => {
        if (!this.user1Token || !this.user2Token) {
          throw new Error('JWT tokens are required. Please set user1Token and user2Token.');
        }

        this.user1Socket = await this.connectUser(1, this.user1Token);
        this.user2Socket = await this.connectUser(2, this.user2Token);
      ...
      await this.runTest('Direct Message Sending', async () => {
        ...
        const messagePromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Message not received')), 10000);
      ...
      await this.runTest('Bidirectional Messaging', async () => {
        const replyPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Reply not received')), 10000);
          
          this.user1Socket.once('newDirectMessage', (data) => {
            clearTimeout(timeout);
            if (data.message.content === 'Hello back from User 2!') {
              resolve(data);
            } else {
        ...
        this.user2Socket.emit('sendDirectMessage', {
          recipientId: this.user1Id,
          type: 'text',
          content: 'Hello back from User 2!'
        }, (response) => {
          if (!response.success) {
            throw new Error(`Reply send failed: ${response.error}`);
          }
        });
      ...
      await this.runTest('Typing Indicators', async () => {
        ...
        const typingPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Typing indicator not received')), 10000);
          
          this.user2Socket.once('userTyping', (data) => {
            clearTimeout(timeout);
            if (data.isTyping && data.conversationId === this.conversationId) {
              resolve(data);
            } else {
      ...
      // Test 5: Voice Call
      await this.runTest('Voice Call Initiation and Response', async () => {
        const callId = `test_call_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

        const callPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Call response not received')), 15000);
          
          this.user1Socket.once('callResponse', (data) => {
            clearTimeout(timeout);
            if (data.response === 'accept' && data.callId === callId) {
              resolve(data);
            } else {
        ...
        this.user1Socket.emit('initiateCall', {
          recipientId: this.user2Id,
          callType: 'voice',
          callId: callId
        }, (response) => {
          if (!response.success) {
            throw new Error(`Call initiation failed: ${response.error}`);
          }
        });
      ...
      await this.runTest('Call Termination', async () => {
        ...
        const endCallPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Call end notification not received')), 10000);
        ...
        this.user1Socket.emit('endCall', {
          callId: this.currentCallId,
          participants: [this.user1Id, this.user2Id]
        }, (response) => {
          if (!response.success) {
            throw new Error(`Call end failed: ${response.error}`);
          }
        });
      ...
      await this.runTest('User Status Updates', async () => {
        const statusPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Status change not received')), 10000);
          
          this.user2Socket.once('userStatusChanged', (data) => {
            clearTimeout(timeout);
            if (data.status === 'away' && data.userId === this.user1Id) {
              resolve(data);
            } else {
        ...
        this.user1Socket.emit('updateStatus', {
          status: 'away'
        }, (response) => {
          if (!response.success) {
      ...
      await this.runTest('Message Read Receipts', async () => {
        ...
        const newMessagePromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('New message not received')), 10000);
        ...
        const readReceiptPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Read receipt not received')), 10000);
      ...
      await this.runTest('Error Handling for Invalid Operations', async () => {
        // Test sending message to non-existent user
        const errorPromise = new Promise((resolve, reject) => {
          const timeout = setTimeout(() => reject(new Error('Error response not received')), 10000);
          ...
          this.user1Socket.emit('sendDirectMessage', {
            ...
            if (!response.success) {
              ...
              reject(new Error('Expected error but operation succeeded'));
            }
          });
        });
...
if (config.user1Token === 'YOUR_USER1_JWT_TOKEN_HERE' || config.user2Token === 'YOUR_USER2_JWT_TOKEN_HERE') {
  ...
  console.log('   2. Or run: curl -X POST http://localhost:3000/auth/login -H "Content-Type: application/json" -d \'{"email":"user@test.com","password":"password123"}\'');
  process.exit(1);
}
