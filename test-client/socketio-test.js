...
class SocketIOTester {
            ...
            socket.on('connect', () => {
                ...
                socket.emit('authenticate', { token }, (response) => {
                    if (response.success) {
                        ...
                        this.log(
                            `User ${userId}: Authenticated as ${response.userId}`
                        )
                        resolve(socket)
                    } else {
                        this.log(
                            `User ${userId}: Authentication failed - ${response.error}`
                        )
    ...
    setupEventListeners(socket, userId) {
        socket.on('authenticated', (data) => {
            this.log(
                `User ${userId}: Authentication successful - ${data.message}`
            )
        })
        ...
        socket.on('newDirectMessage', (data) => {
            this.log(
                `User ${userId}: Received direct message in conversation ${data.conversationId}`
            )
        ...
        socket.on('newGroupMessage', (data) => {
            this.log(
                `User ${userId}: Received group message in group ${data.groupId}`
            )
            this.log(`  Message: ${JSON.stringify(data.message)}`)
        })
        ...
        socket.on('userTyping', (data) => {
            this.log(
                `User ${userId}: ${data.userName} is ${data.isTyping ? 'typing' : 'stopped typing'}`
            )
        })

        socket.on('userStatusChanged', (data) => {
            this.log(
                `User ${userId}: User ${data.userId} status changed to ${data.status}`
            )
        })

        socket.on('userJoinedGroup', (data) => {
            this.log(
                `User ${userId}: ${data.user.name} joined group ${data.groupId}`
            )
        })

        socket.on('userLeftGroup', (data) => {
            this.log(
                `User ${userId}: ${data.user.name} left group ${data.groupId}`
            )
        })

        socket.on('incomingCall', (data) => {
            this.log(
                `User ${userId}: Incoming ${data.callType} call from ${data.caller.name}`
            )
        ...
        socket.on('callResponse', (data) => {
            this.log(
                `User ${userId}: Call ${data.response}ed by ${data.responder.name}`
            )
        })
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.emit(
                ...
                (response) => {
                    if (response.success) {
                        this.log(
                            `✅ Direct message test passed: ${senderId} -> ${recipientId}`
                        )
                        resolve(response)
                    } else {
                        this.log(
                            `❌ Direct message test failed: ${response.error}`
                        )
                        reject(new Error(response.error))
                    }
                }
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.emit(
                ...
                (response) => {
                    if (response.success) {
                        this.log(
                            `✅ Group message test passed: ${senderId} -> Group ${groupId}`
                        )
                        resolve(response)
                    } else {
                        this.log(
                            `❌ Group message test failed: ${response.error}`
                        )
                        reject(new Error(response.error))
                    }
                }
    ...
    async testJoinGroup(userId, groupId) {
        return new Promise((resolve, reject) => {
            ...
            socket.emit('joinGroup', { groupId }, (response) => {
                if (response.success) {
                    this.log(
                        `✅ Join group test passed: ${userId} joined Group ${groupId}`
                    )
                    resolve(response)
                } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.emit(
                ...
                (response) => {
                    if (response.success) {
                        this.log(
                            `✅ Voice call test passed: ${callerId} -> ${recipientId}`
                        )
                        resolve(response)
                    } else {
    ...
    async testStatusUpdate(userId, status) {
        return new Promise((resolve, reject) => {
            ...
            socket.emit('updateStatus', { status }, (response) => {
                if (response.success) {
                    this.log(
                        `✅ Status update test passed: ${userId} -> ${status}`
                    )
                    resolve(response)
                } else {
...
if (require.main === module) {
    console.log(
        '⚠️  To run tests, uncomment the test lines in runTests() function'
    )
